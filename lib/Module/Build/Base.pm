...
sub feature      { shift()->{phash}{features}->access(@_) }
...
sub has_config_data {
  my $self = shift;
  return scalar grep $self->{phash}{$_}->has_data(), qw(config_data features);
}
...
sub write_config {
  ...
  $self->{phash}{$_}->write() foreach qw(notes cleanup features config_data runtime_params);
}
